...
data = [f.read(1024).split(',')] # data[0]에는 의미 없는 단어가 있다
								 # python에서는 문자열을 []안에 넣어 저장하면 문자열 자체가 배열로 저장됨
								 # data = ["abc"]
								 # data[0][1] #--> b
f.close()
...
# 보조 기억 장치를 연다
word_freqs = touchopen('word_freqs', 'rb+') # file open modes => http://www.manpagez.com/man/3/fopen/
# 입력 파일을 연다
f = open(sys.argv[1])
# 입력 파일 내의 각 줄을 순회
while True:
...
while True:
	...
	for i in range(25): # 기호 i를 ㅔㅈ거하는 것은 연습 문제로 남긴다
